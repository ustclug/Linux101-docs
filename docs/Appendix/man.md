...
必须承认的是，尽管 man 文档有一些有中文翻译（安装 `manpages-zh` 软件包即可），但是阅读它们，仍然是一件非常痛苦的事情。况且中文翻译有一些也已经十分老旧（例如，`execve()` 文档的最新翻译仍然在 2003 年 5 月），且有一些错误（例如，`man(1)` 的文档中介绍 `-a|-b` 的含义的时候，将 "cannot be used together" 翻译为了「可以一起使用」）。所以建议有能力的同学尽可能阅读英文版本的 man 文档。
