...
如果 tty/pty 被关闭，则前后台进程一起收到 SIGHUP，如果 shell 被 kill -9 （SIGTERM 无效），则后台进程保留被接管。注意，shell可以主动退出，这样后台进程不收 SIGHUP。但同时注意，ssh 的初始shell不适用于此情形，如果该 shell 主动退出，不会保留后台进程。
