...
!!! Warning "本文可能基本完稿，尚需补充和修订，不是正式版本。"
...
- 使用 `{ 命令1; 命令2; … }`，组命令在 shell 内执行，不会产生新的进程，注意括号和命令之间的空格。

- 使用 `(命令1; 命令2; …)`，组命令会建立独立的 shell 子进程来执行组命令。
...
!!! question "Shell 脚本编写练习 #2"
    ...
    提示：计算平方数可以使用 `bc`，也可以自己写循环计算，假设输入 x 是数字且不大。
