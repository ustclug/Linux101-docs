...
[AOSP (Android Open Source Project)](https://source.android.com/) 只使用了 GPL 许可证的 Linux Kernel，而在 Kernel 之上的 [ART (Android Runtime)](https://source.android.com/docs/core/runtime)、Bionic C 库、驱动透明化的 [HAL (Hardware Abstraction Layer)](https://source.android.com/docs/core/architecture/hal) 则作为用户态存在，避免 Android 系统框架、Google 移动应用服务框架（GMS）和各厂商的驱动程序被 GPL 感染而开源。
