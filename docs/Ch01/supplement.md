...
如果对显示效果不满意，可以采取以下方式调整。

#### 调整虚拟化软件的缩放策略 {#vm-hypervisor-scale-adjust}

VirtualBox 可以在启动后调整缩放策略。点击菜单栏的「查看」->「虚拟显示屏 1」，可以看到缩放比例选项，并且有两种包含了备注的比例：
...
VMware Workstation (Windows) 中其会尝试自行处理缩放问题，取消选择「在虚拟机中自动调整用户界面大小」，并启用拉伸模式后可以在虚拟机中设置较低的分辨率来实现后者的效果。免费版本的 Workstation Player 不支持拉伸模式。
...
在「外观」->「字体」中可以调整元素 DPI。默认情况下，Xubuntu 20.04 的 DPI 是 96，将其乘以预期的缩放比并输入。
...
#### 在 VirtualBox 下闪屏 {#virtualbox-flicker}

我们发现在 macOS 版本的 VirtualBox 中，可能会出现闪屏的问题。在虚拟机设置中将显示控制器修改为 VMSVGA 后可以解决此问题。
